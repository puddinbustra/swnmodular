...
Hooks.on('preCreateChatMessage', (msg, options, userId) => {
  ...
  if(msg.data.flags){
    skillId = msg.data.flags.swnpretty?.roll?.skillId;
    saveId = msg.data.flags.swnpretty?.roll?.saveId;
    isInitiative = msg.data.flags.core?.initiativeRoll;
    // isDeathSave = msg.data.flags.swnpretty?.roll?.type === "death";
    isAttack = msg.data.flags.swnpretty?.roll?.type === "attack";
  }

  // Blind-ify Default 5e & MARS 5e Roll Cards
  // We check the swnpretty flags in the msg to see if it contains any of the skills,
