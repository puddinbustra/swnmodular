...
export default class Item5e extends Item {
  ...
  _getUsageUpdates({consumeQuantity, consumeRecharge, consumeResource, consumeSpellLevel, consumeUsage}) {
    ...
    if ( consumeSpellLevel ) {
      ...
      if ( spells === 0 ) {
        const label = game.i18n.localize(consumeSpellLevel === "pact" ? "SWNPRETTY.SpellProgPact" : `SWNPRETTY.SpellLevel${id.level}`);
  ...
  async displayCard({rollMode, createMessage=true}={}) {
    ...
    const html = await renderTemplate("systems/swnpretty/templates/chat/item-card.html", templateData);
  ...
  static async createScrollFromSpell(spell) {
    ...
    // Get scroll data
    const scrollUuid = `Compendium.${CONFIG.SWNPRETTY.sourcePacks.ITEMS}.${CONFIG.SWNPRETTY.spellScrollIds[level]}`;
