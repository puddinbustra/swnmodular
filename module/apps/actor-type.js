...
export default class ActorTypeConfig extends FormApplication {
  ...
  getData(options) {
    ...
    if ( foundry.utils.getType(attr) !== "Object" ) attr = {
      value: (attr in CONFIG.SWNPRETTY.creatureTypes) ? attr : "humanoid",
      subtype: "",
      swarm: "",
      custom: ""
    };
    ...
    // Populate choices
    const types = {};
    for ( let [k, v] of Object.entries(CONFIG.SWNPRETTY.creatureTypes) ) {
      types[k] = {
    ...
    return {
      ...
      custom: {
        value: attr.custom,
        label: game.i18n.localize("SWNPRETTY.CreatureTypeSelectorCustom"),
        chosen: attr.value === "custom"
      },
      subtype: attr.subtype,
      swarm: attr.swarm,
      sizes: Array.from(Object.entries(CONFIG.SWNPRETTY.actorSizes)).reverse().reduce((obj, e) => {
        obj[e[0]] = e[1];
        return obj;
      }, {}),
