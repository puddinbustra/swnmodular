...
export default class SelectItemsPrompt extends Dialog {
  constructor(items, dialogData={}, options={}) {
    ...
    this.options.classes = ["SWNPRETTY", "dialog", "select-items-prompt", "sheet"];
  ...
  static async create(items, {
    ...
    const html = await renderTemplate("systems/swnpretty/templates/apps/select-items-prompt.html", {items, hint});

    return new Promise((resolve) => {
      const dlg = new this(items, {
        title: game.i18n.localize('SWNPRETTY.SelectItemsPromptTitle'),
        content: html,
        buttons: {
          apply: {
            icon: `<i class="fas fa-user-plus"></i>`,
            label: game.i18n.localize('SWNPRETTY.Apply'),
            callback: html => {
          ...
          cancel: {
            ...
            label: game.i18n.localize('SWNPRETTY.Skip'),
            callback: () => resolve([])
          }
        },
        default: "apply",
